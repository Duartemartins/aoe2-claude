...
function SortableRecommendation({ rec, idx, hook, hookNames, onMoveRecommendation, onRemoveRecommendation }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="border-b border-aoe-gold/5 last:border-b-0 group/rec">
      <div className="flex items-center gap-2 px-3 pt-2">
        <button
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing p-1 text-aoe-parchment/30 hover:text-aoe-parchment/60"
...
export default function UnitPanel({ unit, civ = 'all', sections = [], quoteSearchQuery = '', isHomeView = false, isRecommendedView = false, isTauntsView = false, recommendedSetup = null, onRemoveRecommendation = null, onMoveRecommendation = null, onReorderRecommendations = null, onAddRecommendation = null, onImportSetup = null, onNavigate = null }) {
  ...
  const handleDownloadAll = async () => {
    ...
    saveAs(content, 'aoe2-claude-sounds.zip');
  ...
  if (quoteSearchQuery) {
    return (
      <>
        <QuoteSearchResults sections={sections} searchQuery={quoteSearchQuery} civ={civ} />
        <SelectionActionBar />
      </>
    );
  }
  ...
  if (isRecommendedView) {
    ...
    const renderHook = (hook) => (
      <div key={hook.name} id={`hook-${hook.name}`} className="mb-8">
        <div className="mb-3">
          <h2 className="font-display text-lg font-semibold text-aoe-gold tracking-wide">{hook.name}</h2>
          <p className="text-sm text-aoe-parchment/35 font-body">{hook.description}</p>
        </div>
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd(hook.name)}>
          <SortableContext items={hook.recommendations.map(r => r.audioUrl)} strategy={verticalListSortingStrategy}>
            <div className="bg-aoe-stone/40 rounded-lg border border-aoe-gold/10">
      ...
      <>
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-6 flex items-start justify-between">
            <div>
              <h1 className="font-display text-2xl font-bold text-aoe-gold mb-2 tracking-wide">Recommended Setup</h1>
              <p className="text-aoe-parchment/40 text-sm font-body">
                Curated AoE2 sounds that work well with Claude Code hooks. Click to preview, download to use.
              </p>
            </div>
            <div className="flex items-center gap-2 relative">
              ...
              <div className="relative">
                <button
                  ...
                  className="p-2 rounded-lg bg-aoe-stone/50 text-aoe-parchment/50 hover:bg-aoe-stone transition-colors"
                  title="More options"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {showSettingsMenu && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-48 bg-aoe-darker border border-aoe-gold/15 rounded-lg shadow-xl z-50 py-1">
                      <button
                        onClick={() => { handleDownloadAll(); setShowSettingsMenu(false); }}
                        disabled={isDownloading}
                        className="w-full flex items-center gap-3 px-4 py-2 text-left text-aoe-parchment/60 hover:bg-aoe-stone/50 disabled:opacity-50 font-body"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-4 py-2 text-left text-aoe-parchment/60 hover:bg-aoe-stone/50 font-body"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <label className="w-full flex items-center gap-3 px-4 py-2 text-left text-aoe-parchment/60 hover:bg-aoe-stone/50 cursor-pointer font-body">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    return (
      <div className="flex-1 flex items-center justify-center text-aoe-parchment/30">
        <div className="text-center">
          ...
          <p className="font-body">Select a unit to view sounds</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="flex-1 overflow-y-auto p-6">
        <h1 className="font-display text-2xl font-bold text-aoe-gold mb-6 tracking-wide">{unit.name}</h1>
