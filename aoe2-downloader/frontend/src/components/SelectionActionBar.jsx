...
function extractFilename(audioUrl) {
  ...
  return urlParts.find(part => /\.(ogg|mp3|wav)$/.test(part)) || 'audio.ogg';
}
...
export default function SelectionActionBar() {
  ...
  const prepareQuotes = () => {
    return selectedArray.map(item => {
      const baseName = extractFilename(item.quote.audioUrl).replace(/\.(ogg|mp3|wav)$/, '');
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-aoe-darker border border-aoe-gold/15 rounded-lg shadow-2xl px-6 py-3 flex items-center gap-4 z-50 backdrop-blur-sm">
      <span className="text-aoe-parchment/60 font-body">
        {selectedCount} sound{selectedCount !== 1 ? 's' : ''} selected
      </span>
      <button
        onClick={clearSelection}
        className="text-aoe-parchment/40 hover:text-aoe-parchment/80 transition-colors font-body"
      >
        Clear
      </button>

      <div className="flex items-center gap-2 border-l border-aoe-gold/10 pl-4">
        <select
          value={selectedFolder}
          onChange={(e) => setSelectedFolder(e.target.value)}
          className="bg-aoe-dark text-aoe-parchment/60 border border-aoe-gold/15 rounded px-2 py-1.5 text-sm font-body"
        >
