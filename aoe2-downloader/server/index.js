...
app.get('/api/audio', async (req, res) => {
  ...
  try {
    ...
    const contentType = url.endsWith('.mp3') ? 'audio/mpeg'
...
// Detect input format from URL extension
...
app.get('/api/download', async (req, res) => {
  ...
  try {
    ...
    const inputFormat = detectInputFormat(url);
...
// Helper function to convert audio to MP3 buffer
function convertToMp3(audioBuffer, url = '') {
...
app.post('/api/download-batch', async (req, res) => {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="aoe2-sounds.zip"');
  ...
  for (const quote of quotes) {
    try {
      ...
      const mp3Buffer = await convertToMp3(audioBuffer, quote.audioUrl);
...
app.post('/api/setup-listener', async (req, res) => {
  ...
  try {
    ...
    const bashWarning = shell === 'bash'
      ? ' Note: The listener script uses zsh syntax. You may need zsh installed.'
      : '';
...
app.get('/api/hooks-status', async (req, res) => {
  ...
  try {
    ...
    const status = {};
    for (const [hookName] of Object.entries(REQUIRED_HOOKS)) {
...
app.listen(PORT, () => {
  console.log(`AoE2 Sounds server running at http://localhost:${PORT}`);
  console.log('');
